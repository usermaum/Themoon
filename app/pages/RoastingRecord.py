...
with tab1:
    ...
    # 필터 변경 감지 및 페이지 초기화
    filter_key = f"{date_filter}_{page_size}_{sort_option}"
    ...
    if filtered_logs:
        ...
        with col3:
            st.markdown(
                ...
                f"페이지 {st.session_state.roasting_page_number + 1} / {total_pages} "
                f"(전체 {total_records}건 중 {start_idx + 1}-{end_idx}번째 표시)"
                f"</div>",
