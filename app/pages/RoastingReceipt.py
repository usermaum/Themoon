...
def calculate_results(df):
    ...
    for idx, row in df.iterrows():
        ...
        if raw_weight > 0:
            ...
            # 상태 판정 (차이가 음수 = 예상보다 손실률 낮음 = 좋은 것)
            if variance <= 0:
                # 예상보다 손실률이 낮거나 같으면 항상 정상
                status = "🟢 정상"
            elif variance <= 3.0:
                # 예상보다 0~3% 높으면 정상
                status = "🟢 정상"
...
# 예상 손실률 자동 계산 버튼
if st.button("🔢 예상 손실률 자동 계산", help="생두와 로스팅 후 무게를 기반으로 예상 손실률을 자동으로 계산합니다"):
    for idx, row in edited_df.iterrows():
        # 빈 행 건너뛰기
        if pd.isna(row["생두(kg)"]) or row["생두(kg)"] == 0:
            continue
        if pd.isna(row["로스팅후(kg)"]) or row["로스팅후(kg)"] == 0:
            continue
...
if len(result_df) > 0:
    ...
    st.caption("🟢 정상 (예상보다 낮거나 +3% 이내) | 🟡 주의 (+3~5%) | 🔴 위험 (+5% 초과)")
else:
