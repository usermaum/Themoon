...
erDiagram
    INVENTORY_LOG {
        int id PK
        int bean_id FK
        string change_type "IN | OUT | ADJUST"
        float quantity_change "+300 | -50"
        float quantity_after "잔여 재고"
        string reason "입고 | 로스팅 | 조정"
        datetime created_at
...
erDiagram
    BEAN {
        ...
        string name
        string origin
        string type "GREEN | ROASTED | BLEND"
        float current_quantity
        float unit_price
        string roast_profile "LIGHT | MEDIUM | DARK"
    }
    ...
    BEAN ||--o{ BLEND_ITEM : "포함"
    BLEND ||--o{ BLEND_ITEM : "구성"
    SUPPLIER ||--o{ INBOUND_DOCUMENT : "공급"
    INBOUND_DOCUMENT ||--o{ INBOUND_ITEM : "상세"
    BEAN ||--o{ INBOUND_ITEM : "매칭"
    BEAN ||--o{ INVENTORY_LOG : "추적"
```
