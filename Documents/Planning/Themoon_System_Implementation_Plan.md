...
### Phase 1: 기반 구축 및 입고 자동화 (Week 1)
- [x] 1. DB 스키마 설계 및 마이그레이션.
- [x] 2. 원두 마스터 데이터(`Beans`, `Recipes`) 초기 적재.
- [/] 3. OCR 연동 모듈 개발 및 입고 등록 페이지 구현.
    - [x] OCR 연동 및 이미지 업로드
    - [x] 입고 등록 UI
    - [x] 중복 방지 (Invoice Number)
    - [ ] 원두 매칭 (Fuzzy Matching) 기능 구현 필요

### Phase 2: 생산 프로세스 구현 (Week 2)
- [/] 1. 로스팅 입력 폼 및 로직 구현 (손실률 계산 포함).
    - [x] Backend API (Roasting Transaction, Cost Calculation)
    - [x] Frontend UI (Roasting Page, Sidebar Link)
2.  블렌딩 생산 로직 및 UI 구현.
3.  재고 수불 로직(`Transactions`) 안정화 및 테스트.

### Phase 3: 대시보드 및 안정화 (Week 3)
1.  재고 현황 대시보드 개발.
