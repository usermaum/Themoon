...
graph TD
    subgraph "Presentation Layer"
        FE["Next.js 14 Frontend<br/>(SSR + Client Components)"]
        FE -->|React 18 + TS| Tools["Tailwind + shadcn/ui"]
        FE -->|Data Fetching| SWR["SWR"]
    end
    ...
    subgraph "Data Layer"
        ...
        Tables["Tables: beans, blends, etc."]
        DB --- Tables
    end

    FE <-->|HTTPS RESTful API| API
...
```mermaid
flowchart TD
    Step1[1. 생두 구매 등록] -->|Bean Management| Reg[생두 등록<br/>20kg, 12,000원]
    Step2[2. 로스팅 작업 실행] -->|Roasting Process| Roast[Single Origin Roasting<br/>Input: 20kg -> Output: 17kg]
    Step3[3. 재고 확인] -->|Inventory Management| Update[재고 자동 업데이트<br/>생두 0kg, 원두 17kg]
...
```mermaid
flowchart TD
    Step1[1. 블렌드 레시피 생성] -->|Blend Management| Recipe[레시피 등록<br/>마사이 40%, 안티구아 40%<br/>모모라 10%, 시다모 10%]
    Step2[2. 블렌드 로스팅 실행] -->|Roasting Process| Roast[Blend Roasting<br/>목표 10kg 생산]
    Step3[3. 원가 자동 계산] -->|Cost Calculation| Cost[가중 평균 원가 산출]
    
    Recipe --> Step2
    Roast -->|원두 비율대로 투입| Step3
```
...
graph TD
    subgraph "Development (Local)"
        FE["Frontend<br/>localhost:3500"]
        BE["Backend<br/>localhost:8000"]
        DB[("SQLite<br/>backend/themoon.db")]
    ...
    subgraph "Production (Render.com)"
        PFE["Frontend Service<br/>Next.js"]
        PAPI["Backend Service<br/>FastAPI"]
        PDB[("PostgreSQL")]
