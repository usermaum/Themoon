...
export async function createBean(data: BeanCreateData): Promise<Bean> {
    ...
    // 목록 캐시 무효화
    await mutate((key) => typeof key === 'string' && key.startsWith(BEANS_KEY), undefined, { revalidate: true })
    return result
}
...
export function refreshAllBeans() {
    return mutate((key) => typeof key === 'string' && key.startsWith(BEANS_KEY), undefined, { revalidate: true })
}
