...
import { DesignLabIndex } from './DesignLabIndex';
import { DesignSampleIndex } from './DesignSampleIndex';
import { RoastingInvoice } from './RoastingInvoice';
import { RoastingConsole } from './RoastingConsole';
...
export function DesignShowcase() {
    ...
    const [selectedTab, setSelectedTabState] = useState<
        'components' | 'layouts' | 'interactions' | 'mascot' | 'dashboard' | 'uikit' | 'lab' | 'invoice' | 'samples' | 'console'
    >('components');

    // Sync state with URL params on mount/update
    useEffect(() => {
        const tab = searchParams.get('tab');
        if (tab && ['components', 'layouts', 'interactions', 'mascot', 'dashboard', 'uikit', 'lab', 'invoice', 'samples', 'console'].includes(tab)) {
            setSelectedTabState(tab as any);
        }
    }, [searchParams]);
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-latte-50 via-white to-latte-100">
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
                ...
                >
                    <div className="bg-white rounded-3xl shadow-lg border border-latte-200 p-2 inline-flex gap-2">
                        {[
                            ...
                            { id: 'mascot' as const, label: '마스코트', icon: Cat },
                            { id: 'dashboard' as const, label: '대시보드', icon: PieChart },
                            { id: 'console' as const, label: '콘솔', icon: Flame },
                            { id: 'uikit' as const, label: 'UI 킷', icon: Palette },
                            { id: 'lab' as const, label: '연구소', icon: Target },
                            { id: 'invoice' as const, label: '송장', icon: Coffee },
                            { id: 'samples' as const, label: '샘플', icon: Package },
                        ].map((tab) => (
                            <Button
                                key={tab.id}
                                variant={selectedTab === tab.id ? 'default' : 'ghost'}
                                className={`text-base px-6 py-6 rounded-2xl transition-all ${selectedTab === tab.id ? 'bg-latte-800 text-white shadow-lg' : 'hover:bg-latte-50'
                                    }`}
                ...
                {/* Global Footer for Demo Page */}
                <footer className="max-w-7xl mx-auto px-6 py-24 border-t border-latte-100 mt-24">
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-12">
                        <div className="md:col-span-2 space-y-6">
                            <div className="flex flex-col">
                                <h2 className="text-3xl font-serif font-bold text-latte-900 tracking-tighter">The Moon</h2>
                                <p className="text-xs font-bold text-latte-400 tracking-[0.4em] uppercase">Digital Roasting Management</p>
                            </div>
                            <p className="text-latte-500 max-w-sm">아날로그의 감성과 디지털의 효율이 만나는 곳. The Moon Design System은 단순한 도구를 넘어 커피 문화를 선도합니다.</p>
                        </div>
                        <div className="space-y-6">
                            <h4 className="font-bold text-latte-900">Resources</h4>
                            <ul className="space-y-3 text-latte-500 text-sm">
                                <li><button onClick={() => setSelectedTab('components')} className="hover:text-latte-900 transition-colors text-left">Documentation</button></li>
                                <li><button onClick={() => setSelectedTab('lab')} className="hover:text-latte-900 transition-colors text-left">Theme Engine</button></li>
                                <li><button onClick={() => setSelectedTab('dashboard')} className="hover:text-latte-900 transition-colors text-left">API References</button></li>
                                <li><button onClick={() => setSelectedTab('uikit')} className="hover:text-latte-900 transition-colors text-left">Component Library</button></li>
                            </ul>
                        </div>
                        <div className="space-y-6">
                            <h4 className="font-bold text-latte-900">Lab Projects</h4>
                            <ul className="space-y-3 text-latte-500 text-sm">
                                <li><button onClick={() => setSelectedTab('lab')} className="hover:text-latte-900 transition-colors text-left">Cafe Latte Art</button></li>
                                <li><button onClick={() => setSelectedTab('lab')} className="hover:text-latte-900 transition-colors text-left">Modern Cold Brew</button></li>
                                <li><button onClick={() => setSelectedTab('lab')} className="hover:text-latte-900 transition-colors text-left">Organic Raw Bean</button></li>
                                <li><button onClick={() => setSelectedTab('lab')} className="hover:text-latte-900 transition-colors text-left">Industrial Factory</button></li>
                            </ul>
                        </div>
                    </div>
                    <div className="mt-24 pt-8 border-t border-latte-50 flex flex-col md:flex-row justify-between items-center gap-4 text-xs font-medium text-latte-400 uppercase tracking-widest text-center md:text-left">
                        <p>© 2025 The Moon Drip Bar. All rights reserved.</p>
                        <p>Developed with Passion & Caffeine</p>
                    </div>
                </footer>
            </div>
        </div>
