...
import { DesignLabClassic } from './DesignLabClassic';
import { DesignLabModern } from './DesignLabModern';
import { DesignLabOrganic } from './DesignLabOrganic';
import { DesignLabIndustrial } from './DesignLabIndustrial';
import { DesignLabArtistic } from './DesignLabArtistic';

export function DesignLabIndex() {
    const searchParams = useSearchParams();
    const project = searchParams.get('project');

    if (project === 'classic') return <DesignLabClassic />;
    if (project === 'modern') return <DesignLabModern />;
    if (project === 'organic') return <DesignLabOrganic />;
    if (project === 'industrial') return <DesignLabIndustrial />;
    if (project === 'artistic') return <DesignLabArtistic />;
    ...
    const demos = [
        ...
        },
        {
            ...
            href: '?tab=lab&project=industrial',
            ...
            <div className="max-w-4xl w-full">
                <div className="text-center mb-12 space-y-4">
                    ...
                    <p className="text-xl text-coffee-600 max-w-2xl mx-auto">
                        Explore 5 distinct design directions for the Roasting Management System. Select a
                        concept to view the interactive demo.
                    </p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {demos.map((demo) => (
                        <Link key={demo.id} href={demo.href} className="group">
                            <div
                                className={`h-full p-8 rounded-2xl shadow-lg transition-all duration-300 transform group-hover:-translate-y-2 group-hover:shadow-2xl ${demo.color} relative overflow-hidden`}
                            >
