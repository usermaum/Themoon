...
function Counter({
  ...
  const rounded = useTransform(count, (latest) => {
    return prefix + formatCurrency(latest) + suffix;
  });
                      ...
                      {inventoryStats?.suppliers?.slice(0, 3).map((s: any) => (
                        <div key={s.name} className="flex items-center justify-between text-sm">
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <span className="font-medium text-blue-600 w-10 text-right">
                  ...
                  <div className="border-t pt-4">
                    ...
                    >
                      {inventoryStats?.top_items?.map((item: any, idx: number) => (
                        ...
                        >
                          <div className="flex items-center gap-3">
                            <span className="flex items-center justify-center text-xs font-bold text-amber-600 bg-amber-100 w-5 h-5 rounded-full">
                              {idx + 1}
                            </span>
          ...
          <TabsContent value="cost" className="space-y-4">
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-2">
              <Card className="col-span-1 rounded-[1em]">
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                  ...
                  <div className="w-[180px]">
                    <Select value={selectedBeanForTrend} onValueChange={setSelectedBeanForTrend}>
                      <SelectTrigger>
                        <SelectValue placeholder="품목 선택" />
                      </SelectTrigger>
                      <SelectContent>
                        {availableBeans.map((bean) => (
                          <SelectItem key={bean} value={bean}>
                            {bean}
                          </SelectItem>
                        ))}
                      </SelectContent>
              ...
              <Card className="col-span-1 rounded-[1em]">
                ...
                <CardContent>
                  <AnalysisBriefing beanName={selectedBeanForTrend} data={costTrends} />
                </CardContent>
              </Card>
            </div>
          </TabsContent>
