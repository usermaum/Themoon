    ...
    return (
        <div className="min-h-screen bg-latte-50 p-8 font-sans relative">
            ...
            <header className="flex justify-between items-center mb-12 pt-12 max-w-6xl mx-auto">
                <div>
                    <h1 className="font-serif text-4xl text-latte-900 font-bold tracking-tight">Morning Dashboard</h1>
                ...
                <div className="flex items-center gap-4">
                    <div className="flex items-center gap-2 bg-white px-4 py-2 rounded-full shadow-sm border border-latte-100">
                        ...
                        <Switch id="store-status" defaultChecked className="data-[state=checked]:bg-green-600" />
                    </div>
            ...
            <main className="max-w-6xl mx-auto space-y-8">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <Card className="bg-white border-none shadow-lg shadow-latte-900/5 hover:-translate-y-1 transition-transform duration-300">
                        <CardHeader className="flex flex-row items-center justify-between pb-2">
                            <CardTitle className="text-sm font-medium text-latte-500 uppercase tracking-widest">Active Orders</CardTitle>
                        ...
                        <CardContent>
                            ...
                            <p className="text-xs text-latte-400 mt-1 flex items-center">
                                <span className="text-green-600 font-bold mr-1 flex items-center"><TrendingUp className="h-3 w-3 mr-0.5" /> +12%</span> from last hour
                            </p>
                        </CardContent>
                    </Card>
                    <Card className="bg-white border-none shadow-lg shadow-latte-900/5 hover:-translate-y-1 transition-transform duration-300">
                        <CardHeader className="flex flex-row items-center justify-between pb-2">
                            <CardTitle className="text-sm font-medium text-latte-500 uppercase tracking-widest">Revenue</CardTitle>
                    ...
                    <Card className="bg-gradient-to-br from-latte-800 to-latte-900 text-white border-none shadow-xl hover:-translate-y-1 transition-transform duration-300 relative overflow-hidden">
                        ...
                        <CardHeader className="flex flex-row items-center justify-between pb-2 relative z-10">
                            <CardTitle className="text-sm font-medium text-latte-200 uppercase tracking-widest">VIP Visits</CardTitle>
                ...
                <Card className="bg-white/80 backdrop-blur-sm border-latte-100">
                    <CardHeader>
                        <CardTitle className="font-serif text-xl text-latte-900">Recent Notifications</CardTitle>
                    </CardHeader>
                    <CardContent className="space-y-4">
                        {[
                            { title: 'New Bean Shipment', desc: 'Ethiopia Sidamo set arrived.', time: '2m ago', type: 'info' },
                            { title: 'Machine Maintenance', desc: 'Group head cleaning required.', time: '1h ago', type: 'warning' },
                            { title: 'Goal Reached', desc: 'Monthly sales target achieved!', time: '3h ago', type: 'success' }
                        ].map((item, i) => (
                            <div key={i} className="flex items-start gap-4 p-3 rounded-xl hover:bg-latte-50 transition-colors">
                                <div className={`mt-1 h-2 w-2 rounded-full shrink-0 ${item.type === 'info' ? 'bg-blue-400' :
                                        item.type === 'warning' ? 'bg-orange-400' : 'bg-green-400'
                                    }`}></div>
