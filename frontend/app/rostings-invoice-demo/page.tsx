...
const INITIAL_BEAN_DATA: BeanRow[] = [
    ...
    { id: 6, code: "", name: "키린야가", type: "Pb", greenAmount1: "", roastedAmount1: "", greenAmount2: "", roastedAmount2: "" },
    { id: 7, code: "Co", name: "후일라", type: "W", greenAmount1: "500", roastedAmount1: "433", greenAmount2: "1000", roastedAmount2: "837" },
    { id: 8, code: "Gu", name: "안티구아", type: "W", greenAmount1: "500", roastedAmount1: "437", greenAmount2: "", roastedAmount2: "" },
    { id: 9, code: "Cos", name: "엘탄케", type: "Rh", greenAmount1: "", roastedAmount1: "", greenAmount2: "", roastedAmount2: "" },
    { id: 10, code: "Br", name: "파인디카트모", type: "N", greenAmount1: "", roastedAmount1: "", greenAmount2: "", roastedAmount2: "" },
    { id: 11, code: "Eth", name: "디카페 SDM", type: "SD", greenAmount1: "", roastedAmount1: "", greenAmount2: "", roastedAmount2: "" },
    { id: 12, code: "Co", name: "디카페 SM", type: "SC", greenAmount1: "500", roastedAmount1: "435", greenAmount2: "500", roastedAmount2: "423" },
    { id: 13, code: "Br", name: "스위스워터", type: "SD", greenAmount1: "", roastedAmount1: "", greenAmount2: "", roastedAmount2: "" },
];
...
const STOCK_SINGLE_DATA = [
    "예가체프", "모모라", "코케허니", "우리가", "AA FAQ", "키린야가",
    "후일라", "안티구아", "엘탄케", "파인디카트모", "디카페 SDM", "디카페 SM", "스위스워터"
];
...
export default function RoastingDemoPage() {
    ...
    const getTypeColor = (type: string) => {
        switch (type) {
            case "W": return "text-blue-600 dark:text-blue-400";
            case "N": return "text-red-600 dark:text-red-400";
            case "Rh": return "text-pink-600 dark:text-pink-400";
                            ...
                            <Table className="w-full text-sm">
                                <TableHeader className="bg-gray-100 dark:bg-gray-800">
                                    <TableRow className="border-b border-border hover:bg-transparent">
                                        <TableHead className="w-8 text-center border-r font-bold text-gray-700 dark:text-gray-300">#</TableHead>
                                        <TableHead className="w-12 text-center border-r font-bold text-gray-700 dark:text-gray-300">Code</TableHead>
                                        <TableHead className="text-left font-bold border-r text-gray-700 dark:text-gray-300">Name</TableHead>
                                        <TableHead className="w-14 text-center border-r font-bold text-gray-700 dark:text-gray-300">Type</TableHead>
                                        <TableHead className="text-right bg-red-50/50 text-red-600 dark:text-red-400 border-r font-bold min-w-[70px]">생두량</TableHead>
                                        <TableHead className="text-right border-r font-bold text-gray-700 dark:text-gray-300 min-w-[70px]">볶은량</TableHead>
                                        <TableHead className="text-right bg-red-50/50 text-red-600 dark:text-red-400 border-r font-bold min-w-[70px]">생두량</TableHead>
                                        <TableHead className="text-right font-bold text-gray-700 dark:text-gray-300 min-w-[70px]">볶은량</TableHead>
                                    </TableRow>
                                </TableHeader>
                                <TableBody className="divide-y">
                                    {beanData.map((bean) => (
                                        <TableRow key={bean.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                                            ...
                                            <TableCell className="p-2 text-center text-xs font-mono">{bean.code}</TableCell>
                                            <TableCell className="p-2 font-medium whitespace-nowrap">{bean.name}</TableCell>
                                            <TableCell className={cn("p-2 text-center font-bold", getTypeColor(bean.type))}>
                                                {bean.type}
                                            </TableCell>
                                            ...
                                            <TableCell className="p-1 text-right">
                                                <Input
                                                    className={cn("h-7 text-right p-1 bg-transparent border-none font-mono focus-visible:ring-1 focus-visible:ring-offset-0", bean.id === 7 ? "text-blue-600 dark:text-blue-400" : "")}
                            ...
                            <Table className="w-full text-sm">
                                <TableHeader className="bg-gray-100 dark:bg-gray-800">
                                    <TableRow>
                                        ...
                                        <TableHead className="p-1 h-8 text-center text-xs text-muted-foreground w-1/4">Notes</TableHead>
                                    </TableRow>
                                </TableHeader>
                                <TableBody className="divide-y">
                                    {ORDER_BLEND_DATA.map((item, idx) => (
                                        <TableRow key={idx}>
                                            <TableCell className="w-12 p-1 text-center font-bold bg-gray-50 dark:bg-gray-800">{item.code}</TableCell>
                                            <TableCell className="p-1 text-left font-medium pl-2">{item.name}</TableCell>
                                            <TableCell className={cn("w-12 p-1 text-center font-bold bg-blue-50 dark:bg-blue-900/10", item.type === 'N' && 'bg-red-50 dark:bg-red-900/10 text-red-600')}>
                                                <span className={getTypeColor(item.type)}>{item.type}</span>
                                            </TableCell>
                                            <TableCell className="p-1">
                                                <Input className="h-6 text-xs bg-transparent border-transparent focus:border-input" placeholder="비고 입력" />
                                            </TableCell>
                                        </TableRow>
                                    ))}
                                </TableBody>
                        ...
                        <Card className="rounded-xl shadow-md overflow-hidden border-0 bg-white dark:bg-zinc-900 border-border">
                            <Table className="w-full text-sm h-full">
                                <TableHeader className="bg-gray-100 dark:bg-gray-700">
                                    <TableRow>
                                        <TableHead className="py-2 px-3 text-left border-b border-r font-bold text-gray-700 dark:text-gray-200 h-9">싱글</TableHead>
                                        <TableHead className="py-2 px-3 text-center border-b font-bold text-gray-700 dark:text-gray-200 h-9">재고</TableHead>
                                    </TableRow>
                                </TableHeader>
                                <TableBody className="divide-y">
                        ...
                        <div className="flex flex-col gap-4">
                            ...
                            <Card className="rounded-xl shadow-md overflow-hidden border-0 bg-white dark:bg-zinc-900 border-border">
                                <Table className="w-full text-sm">
                                    <TableHeader className="bg-gray-100 dark:bg-gray-700">
                                        <TableRow>
                                            <TableHead className="py-2 px-2 text-left border-b border-r font-bold text-gray-700 dark:text-gray-200 text-xs h-9">신,탄</TableHead>
                                            <TableHead className="py-2 px-2 text-center border-b border-r font-bold text-gray-700 dark:text-gray-200 text-xs h-9">재고</TableHead>
                                            <TableHead className="py-2 px-2 text-center border-b border-r font-bold text-gray-700 dark:text-gray-200 text-xs h-9">기타</TableHead>
                                            <TableHead className="py-2 px-2 text-center border-b font-bold text-gray-700 dark:text-gray-200 text-xs h-9">재고</TableHead>
                                        </TableRow>
                                    </TableHeader>
                                    <TableBody className="divide-y">
                                        <TableRow className="h-9">
                                            <TableCell className="px-2 py-1 font-medium border-r text-xs">산토스</TableCell>
                                        ...
                                        <TableRow className="h-9">
                                            <TableCell className="px-2 py-1 font-medium border-r text-xs">시다모G4</TableCell>
                                            <TableCell className="px-0 py-0 bg-gray-50 dark:bg-gray-800 border-r">
                                                <Input className="h-full w-full border-none bg-transparent text-center focus-visible:ring-0 text-xs" />
                                            </TableCell>
                                            <TableCell className="px-0 py-0 border-r">
                            ...
                            <Card className="rounded-xl shadow-md overflow-hidden border-0 bg-white dark:bg-zinc-900 border-border">
                                <div className="bg-gray-200 dark:bg-gray-700 py-1 px-3 text-sm font-bold border-b text-center text-gray-800 dark:text-gray-200 flex items-center justify-between">
                                    <span>뉴문 블랜드</span>
                                    <Input className="w-16 h-6 text-center text-xs bg-white dark:bg-gray-800" placeholder="(  )" />
                                </div>
                                    ...
                                    <TableBody className="divide-y">
                                        {["산토스", "후일라", "시디모G4"].map((item, idx) => (
                                            <TableRow key={idx} className="h-8">
                                                <TableCell className="px-2 py-1 border-r w-1/2 text-xs font-medium">{item}</TableCell>
                                ...
                                <Table className="w-full text-sm">
                                    <TableBody className="divide-y">
                                        {["co디카페인", "Br디카페인"].map((item, idx) => (
                                            <TableRow key={idx} className="h-8">
                                                <TableCell className="px-2 py-1 border-r w-1/3 text-xs font-medium">{item}</TableCell>
