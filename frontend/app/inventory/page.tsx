...
export default function InventoryPage() {
    ...
    const logTab = searchParams.get('logTab') || 'all'
    ...
    const handleDelete = (log: InventoryLog) => {
        setLogToDelete(log)
    }
    ...
    const confirmDelete = async () => {
        ...
        try {
            await InventoryLogAPI.delete(logToDelete.id)
            await fetchBeans(beanPage, activeTab, debouncedBeanSearch)
            await fetchLogs(logPage, logTab, debouncedLogSearch)
            setLogToDelete(null)
        } catch (err: any) {
            ...
            <div className="container mx-auto px-4 py-8">
                    ...
                    <Tabs value={activeTab} onValueChange={handleTabChange} className="w-full">
                        <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
                            <TabsList className="bg-white/60 backdrop-blur-md p-1 rounded-full border border-latte-200 shadow-sm h-auto inline-flex">
                                <TabsTrigger value="all" className="rounded-full px-4 py-2 text-sm data-[state=active]:bg-latte-900 data-[state=active]:text-white transition-all">전체 보기</TabsTrigger>
                                <TabsTrigger value="green" className="rounded-full px-4 py-2 text-sm data-[state=active]:bg-green-600 data-[state=active]:text-white transition-all">생두 (Green)</TabsTrigger>
                                <TabsTrigger value="roasted" className="rounded-full px-4 py-2 text-sm data-[state=active]:bg-latte-600 data-[state=active]:text-white transition-all">원두 (Roasted)</TabsTrigger>
                                <TabsTrigger value="blend" className="rounded-full px-4 py-2 text-sm data-[state=active]:bg-amber-600 data-[state=active]:text-white transition-all">블렌드 (Blend)</TabsTrigger>
                            </TabsList>

                            <motion.div layout className="relative w-full md:w-72 group">
                                ...
                                />
                                {beanSearch && (
                                    <button onClick={() => setBeanSearch('')} className="absolute right-4 top-1/2 -translate-y-1/2 text-latte-400 hover:text-latte-600 focus:outline-none p-1 bg-latte-50 rounded-full">
                                        <X className="w-3 h-3" />
                                    </button>
                                )}
                            </motion.div>
                        ...
                        {['all', 'green', 'roasted', 'blend'].map((tabValue) => (
                            <TabsContent key={tabValue} value={tabValue} className="mt-0 animate-in fade-in slide-in-from-bottom-4 duration-500">
                                {loadingBeans ? (
                                    <div className="flex justify-center py-20">
                                        <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-latte-600"></div>
                                    </div>
                                ) : (
                                    <>
                                        <div className="md:hidden space-y-3">
                                            {/* Mobile View Placeholder - simplified for now, as Table handles desktop nicely. 
                                                If needed, we can create InventoryListMobile component later. 
                                                For now, reusing Table for consistency or hiding it on mobile? 
                                                Original had explicit mobile card layout. Let's start with Desktop Table working. */}
                                            <div className="bg-white p-4 rounded-lg text-center text-latte-500">
                                                모바일 뷰는 준비중입니다. 데스크탑에서 확인해주세요.
                                            </div>
                                        </div>
                                        ...
                                        {beanTotalPages > 1 && (
                                            <div className="flex justify-center mt-6 gap-2">
                                                {Array.from({ length: beanTotalPages }, (_, i) => i + 1).map((p) => (
                                                    <Button
                                                        key={p}
                                                        variant={p === beanPage ? "default" : "outline"}
                                                        size="sm"
                                                        onClick={() => updatePage('beanPage', p)}
                                                        className={p === beanPage ? "bg-latte-600 hover:bg-latte-700" : ""}
                                                    >
                                                        {p}
                                                    </Button>
                                                ))}
                    ...
                    <Tabs value={logTab} onValueChange={handleLogTabChange} className="mb-2">
                        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-[0.5em]">
                            ...
                            <div className="relative w-full md:w-64">
                                ...
                                />
                                {logSearch && (
                                    <button onClick={() => setLogSearch('')} className="absolute right-3 top-1/2 -translate-y-1/2 text-latte-400 hover:text-latte-600 focus:outline-none">
                                        <X className="w-4 h-4" />
                                    </button>
                                )}
                            </div>
                                    ...
                                    <div className="bg-white rounded-[1em] shadow-sm overflow-hidden border border-latte-200">
                                        <div className="overflow-x-auto">
                                            <table className="min-w-full divide-y divide-latte-100">
                                                ...
                                                <tbody className="bg-white divide-y divide-latte-100">
                                                    {logs.length === 0 ? (
                                                        <tr><td colSpan={6} className="px-6 py-12 text-center text-latte-400">기록이 없습니다.</td></tr>
                                                    ) : logs.map((log) => (
                                                        <tr key={log.id} className="hover:bg-latte-50/30 transition-colors">
                                                            ...
                                                            <td className="hidden md:table-cell px-6 py-4 whitespace-nowrap">
                                                                <Badge variant={log.change_amount >= 0 ? 'default' : 'destructive'} className={log.change_amount >= 0 ? 'bg-green-600' : ''}>
                                                            ...
                                                            <td className="hidden md:table-cell px-6 py-4 whitespace-nowrap text-sm text-latte-500">{log.notes || '-'}</td>
                                                            <td className="hidden md:table-cell px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                                                                ...
                                                                <Button size="icon" variant="ghost" onClick={() => handleDelete(log)} className="text-latte-400 hover:text-red-600 hover:bg-red-50">
                                        ...
                                        {logs.length > 0 && (
                                            <div className="flex justify-center items-center py-4 gap-2 bg-latte-50/30 border-t border-latte-100">
                                                <Button variant="outline" size="sm" onClick={() => updatePage('logPage', Math.max(1, logPage - 1))} disabled={logPage === 1}>이전</Button>
                                                <span className="text-sm font-medium text-latte-600">{logPage} / {logTotalPages || 1}</span>
                                                <Button variant="outline" size="sm" onClick={() => updatePage('logPage', Math.min(logTotalPages || 1, logPage + 1))} disabled={logPage >= (logTotalPages || 1)}>다음</Button>
                                            </div>
                                        )}
                                    </div>
                                )}
                    ...
                    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 animate-in fade-in duration-200 p-4">
                        <div className="bg-white rounded-[2rem] p-6 md:p-8 w-full max-w-md shadow-2xl scale-100 animate-in zoom-in-95 duration-200 border border-latte-200">
                            <div className="flex justify-between items-center mb-6">
                                <h2 className="text-xl md:text-2xl font-serif font-bold text-latte-900">{transactionType === 'IN' ? '입고' : '출고'} 처리</h2>
                                <Button variant="ghost" size="icon" onClick={closeModal} className="rounded-full hover:bg-latte-100"><X className="w-5 h-5 text-latte-500" /></Button>
                            </div>
                            <form onSubmit={handleSubmit} className="space-y-6">
                                <div><label className="block text-sm font-medium text-latte-700 mb-2">수량 (kg) *</label><Input type="number" required min="0.1" step="0.1" value={quantity} onChange={(e) => setQuantity(e.target.value)} placeholder="예: 10.0" className="font-mono text-lg" autoFocus /></div>
                                <div><label className="block text-sm font-medium text-latte-700 mb-2">사유</label><Input type="text" value={reason} onChange={(e) => setReason(e.target.value)} placeholder="예: 신규 구매, 로스팅 사용" /></div>
                                <div className="flex justify-end gap-3 mt-8">
                                    <Button type="button" variant="outline" onClick={closeModal}>취소</Button>
                                    <Button type="submit" disabled={submitting} className={`${transactionType === 'IN' ? 'bg-green-600 hover:bg-green-700' : 'bg-red-600 hover:bg-red-700'} text-white`}>{submitting ? '처리 중...' : '확인'}</Button>
                                </div>
                            </form>
                        </div>
                    </div>
                ...
                {showEditModal && selectedLog && (
                    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 animate-in fade-in duration-200 p-4">
                        <div className="bg-white rounded-[2rem] p-6 md:p-8 w-full max-w-md shadow-2xl scale-100 animate-in zoom-in-95 duration-200 border border-latte-200">
                            <div className="flex justify-between items-center mb-6">
                                <h2 className="text-xl md:text-2xl font-serif font-bold text-latte-900">수량/사유 수정</h2>
                                <Button variant="ghost" size="icon" onClick={closeEditModal} className="rounded-full hover:bg-latte-100"><X className="w-5 h-5 text-latte-500" /></Button>
                            </div>
                            <form onSubmit={handleEdit} className="space-y-6">
                                <div><label className="block text-sm font-medium text-latte-700 mb-2">수량 (kg) *</label><Input type="number" required min="0.1" step="0.1" value={editQuantity} onChange={(e) => setEditQuantity(e.target.value)} placeholder="예: 10.0" className="font-mono text-lg" /></div>
                                <div><label className="block text-sm font-medium text-latte-700 mb-2">사유</label><Input type="text" value={editReason} onChange={(e) => setEditReason(e.target.value)} placeholder="예: 사유 수정" /></div>
                                <div className="flex justify-end gap-3 mt-8">
                                    <Button type="button" variant="outline" onClick={closeEditModal}>취소</Button>
                                    <Button type="submit" disabled={submitting}>{submitting ? '처리 중...' : '수정 완료'}</Button>
                                </div>
                            </form>
                        </div>
                    </div>
                ...
                <AlertDialog open={!!logToDelete} onOpenChange={(open) => !open && setLogToDelete(null)}>
                    <AlertDialogContent className="bg-white rounded-[2rem] border border-latte-200">
                        <AlertDialogHeader>
                            <AlertDialogTitle className="text-latte-900 font-serif">기록 삭제</AlertDialogTitle>
                            <AlertDialogDescription className="text-latte-600">정말로 이 입출고 기록을 삭제하시겠습니까?<br />삭제 후에는 복구할 수 없습니다.</AlertDialogDescription>
                        </AlertDialogHeader>
                        <AlertDialogFooter>
                            <AlertDialogCancel onClick={() => setLogToDelete(null)} className="rounded-full border-latte-200 text-latte-600 hover:bg-latte-50 hover:text-latte-900">취소</AlertDialogCancel>
                            <AlertDialogAction onClick={confirmDelete} className="rounded-full bg-red-600 hover:bg-red-700 text-white border-none">삭제</AlertDialogAction>
                        </AlertDialogFooter>
                    </AlertDialogContent>
                </AlertDialog>
            </div>
