...
export default function OrganicRawBeanPage() {
    return (
        <div className="fixed inset-0 z-[100] flex bg-[#F0EBE5] font-serif text-[#5D5C56] overflow-hidden">
            ...
            <aside className="w-64 bg-[#E6E1DB] flex flex-col border-r border-[#D8D3CD]">
                <div className="p-8 flex items-center gap-3">
                    <Link href="/design-lab" className="hover:bg-[#D8D3CD] p-2 rounded-full transition-colors">
...
function NavItem({ icon, label, active = false }: { icon: React.ReactNode, label: string, active?: boolean }) {
    return (
      ...
      ${active
                ? 'bg-[#7A9A7E] text-[#F0EBE5]'
                : 'text-[#8C8B85] hover:text-[#4A4945]'}
    `}>
            {icon}
...
function StatCard({ title, value, sub, isWarning = false }: { title: string, value: string, sub: string, isWarning?: boolean }) {
    return (
...
function BeanCard({ name, origin, roast, notes, stock, image }: { name: string, origin: string, roast: string, notes: string, stock: string, image: string }) {
    return (
        <div className="group cursor-pointer">
            ...
            <div className="px-2">
                <h4 className="font-serif text-xl text-[#4A4945] mb-1 group-hover:text-[#7A9A7E] transition-colors">{name}</h4>
