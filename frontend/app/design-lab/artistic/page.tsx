...
export default function ArtisticDemoPage() {
    const [currentTheme, setCurrentTheme] = useState('bauhaus')

    return (
        <div className={`min-h-screen transition-colors duration-500 p-8 font-sans ${currentTheme === 'noir' || currentTheme === 'neon' ? 'bg-neutral-900 text-white' : 'bg-[#faf9f6] text-neutral-800'
            }`}>
            {/* Header / Theme Switcher */}
            <header className="max-w-7xl mx-auto mb-16">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-8">
                    <div>
                        <h1 className="text-4xl md:text-5xl font-bold mb-4 tracking-tight">Artistic Components</h1>
                        <p className={`text-lg max-w-xl ${currentTheme === 'noir' || currentTheme === 'neon' ? 'text-neutral-400' : 'text-neutral-500'}`}>
                            UI를 넘어 예술적인 경험을 제공하는 컴포넌트 컬렉션입니다.<br />
                            각 테마는 단순한 스타일링을 넘어 독특한 분위기를 자아냅니다.
                        </p>
                    </div>
                </div>
                ...
                <div className="flex flex-wrap gap-4 mt-8">
                    {themes.map((theme) => (
                        ...
                        >
                            ...
                            {currentTheme === theme.id && (
                                <span className={`absolute inset-0 opacity-20 ${theme.id === 'bauhaus' ? 'bg-yellow-400' :
                                    theme.id === 'watercolor' ? 'bg-blue-300' :
                                        theme.id === 'noir' ? 'bg-white' :
                                            theme.id === 'collage' ? 'bg-orange-300' :
                                                'bg-purple-500'
                                    }`}></span>
                            )}
                        </button>
            ...
            <main className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-8 items-start">
                ...
                <section className="space-y-4">
                    <h2 className="text-sm font-bold uppercase tracking-widest opacity-50 mb-4">01. Profile Card</h2>
                    {
                        currentTheme === 'bauhaus' ? <ProfileBauhaus /> :
                            currentTheme === 'watercolor' ? <ProfileWatercolor /> :
                                currentTheme === 'noir' ? <ProfileNoir /> :
                                    currentTheme === 'collage' ? <ProfileCollage /> :
                                        <ProfileNeon />
                    }
                </section>
                ...
                {/* 2. Media Player */}
                <section className="space-y-4">
                    <h2 className="text-sm font-bold uppercase tracking-widest opacity-50 mb-4">02. Media Player</h2>
                    {
                        currentTheme === 'bauhaus' ? <PlayerBauhaus /> :
                            currentTheme === 'watercolor' ? <PlayerWatercolor /> :
                                currentTheme === 'noir' ? <PlayerNoir /> :
                                    currentTheme === 'collage' ? <PlayerCollage /> :
                                        <PlayerNeon />
                    }
                </section>
                ...
                <section className="space-y-4">
                    <h2 className="text-sm font-bold uppercase tracking-widest opacity-50 mb-4">03. Content Card</h2>
                    {
                        currentTheme === 'bauhaus' ? <ContentBauhaus /> :
                            currentTheme === 'watercolor' ? <ContentWatercolor /> :
                                currentTheme === 'noir' ? <ContentNoir /> :
                                    currentTheme === 'collage' ? <ContentCollage /> :
                                        <ContentNeon />
                    }
                </section>
            </main>
        ...
        <div className="bg-[#f0f0f0] p-8 relative overflow-hidden group hover:shadow-xl transition-shadow border-2 border-black">
            ...
            <div className="relative z-10 flex flex-col items-center">
                <div className="w-24 h-24 bg-white border-2 border-black rounded-full mb-4 flex items-center justify-center overflow-hidden">
                    <img src="/images/beans/ethiopia.png" alt="Profile" className="w-full h-full object-cover grayscale contrast-125" />
                </div>
        ...
        <div className="bg-white border-2 border-black p-6 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] relative">
            ...
            <div className="flex items-center justify-between mb-6">
                <div>
                    ...
                    <p className="text-xs font-bold uppercase mt-1 bg-black text-white inline-block px-1">Ensemble</p>
                </div>
        ...
        <div className="border-2 border-black bg-white group hover:-translate-y-2 transition-transform duration-300">
            ...
            <div className="p-6 relative">
                ...
                <p className="mt-3 text-sm font-medium leading-relaxed border-l-4 border-red-600 pl-4">
                    Form follows function. Minimalism is not a lack of something. It’s simply the perfect amount of something.
                </p>
        ...
        <div className="bg-white/50 backdrop-blur-sm p-8 rounded-[3rem] shadow-[0_20px_40px_-15px_rgba(0,0,0,0.05)] border border-white/60 relative overflow-hidden text-center font-serif">
            ...
            <div className="relative z-10 flex flex-col items-center">
                <div className="w-28 h-28 rounded-full p-1 bg-gradient-to-br from-blue-200 to-purple-200 mb-4">
                    <img src="/images/beans/ethiopia.png" alt="Profile" className="w-full h-full rounded-full object-cover border-4 border-white" />
                </div>
                ...
                <div className="flex gap-4 mt-8">
                    <button className="px-6 py-2 rounded-full bg-blue-50 text-blue-800 text-sm hover:bg-blue-100 transition-colors">Follow</button>
                    <button className="px-6 py-2 rounded-full bg-white border border-slate-200 text-slate-600 text-sm hover:bg-slate-50 transition-colors">Message</button>
                </div>
            </div>
        </div>
    )
        ...
        <div className="font-serif bg-white/80 backdrop-blur-md rounded-[2rem] p-6 shadow-sm border border-white/50 relative overflow-hidden">
            ...
            <div className="relative z-10">
                ...
                <div className="flex justify-between items-center px-4">
                    <button className="text-slate-400 hover:text-slate-600"><Star size={20} /></button>
        ...
        <div className="group font-serif relative">
            ...
            <div className="relative bg-white rounded-xl p-6 shadow-sm border border-slate-100">
                ...
                <p className="text-lg text-slate-700 leading-relaxed italic mb-6">
                    Color is my day-long obsession, joy and torment. To me, color is a vital energy that must be expressed.
                </p>
        ...
        <div className="bg-black border border-neutral-700 p-1">
            <div className="border border-neutral-800 p-6 flex flex-col h-full bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-neutral-800 via-black to-black">
                ...
                <h3 className="text-2xl font-serif text-white mb-4 leading-snug">
                    The Case of the<br />Missing Roast
                </h3>
                <p className="text-neutral-500 text-sm mb-8 flex-1">
                    Evidence suggests it wasn't an accident. The beans were green when they entered the warehouse...
                </p>
    ...
    return (
        <div className="bg-[#f2e6d6] p-4 rotate-1 shadow-lg relative max-w-sm mx-auto">
            ...
            <div className="bg-[url('https://www.transparenttextures.com/patterns/paper.png')] p-6 border-4 border-white shadow-inner flex flex-col items-center">
                ...
                <h3 className="font-serif text-3xl text-[#2c2c2c] bg-white px-2 -rotate-2 shadow-sm font-bold">The Collector</h3>
                <p className="mt-2 font-mono text-xs text-gray-500 text-center uppercase tracking-widest border-t border-b border-gray-400 py-1 w-full">Est. 2024</p>
            </div>
        ...
        <div className="bg-[#4a4a4a] p-1 shadow-lg -rotate-1 relative">
            ...
            <div className="bg-[#e8e8e8] p-6 h-full flex items-center gap-4 relative overflow-hidden">
                ...
                <div className="w-20 h-20 bg-black flex-shrink-0 border-4 border-white shadow-md relative">
                    ...
                    <div className="w-full h-full flex items-center justify-center text-white font-mono text-2xl">A</div>
                </div>
        ...
        <div className="relative p-4">
            ...
            <div className="relative z-10 p-4">
                <div className="flex gap-2 mb-4 bg-yellow-100 p-2 -ml-6 w-[110%] shadow-sm rotate-1">
                    ...
                    <span className="font-mono text-xs font-bold text-orange-800 uppercase">Featured Story</span>
                </div>
...
function ProfileNeon() {
    return <div className="p-8 border border-purple-500/50 bg-black shadow-[0_0_20px_rgba(168,85,247,0.2)] rounded-lg text-center">
        ...
        <h3 className="text-white font-bold text-xl tracking-wider shadow-cyan-500 drop-shadow-[0_0_5px_rgba(255,255,255,0.5)]">CYBER</h3>
        <p className="text-purple-400 text-xs mt-2 uppercase tracking-[0.2em] animate-pulse">Punk 2077</p>
    </div>;
}
function PlayerNeon() {
    return <div className="p-6 border border-pink-500/50 bg-black/80 rounded-lg flex items-center justify-between">
        <div className="text-cyan-400 font-mono">03:24</div>
        <div className="h-1 bg-gray-800 flex-1 mx-4 relative overflow-hidden"><div className="absolute left-0 top-0 h-full w-1/2 bg-pink-500 shadow-[0_0_10px_#ec4899]"></div></div>
        <Music className="text-pink-500 drop-shadow-[0_0_5px_rgba(236,72,153,1)]" />
    </div>;
}
function ContentNeon() {
    return <div className="p-6 border border-blue-500/30 bg-gray-900/50 rounded-lg relative overflow-hidden group">
