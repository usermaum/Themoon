    ...
    <div className="space-y-6">
      <div className="grid grid-cols-2 gap-4">
        <div className="p-3 bg-red-50 rounded-xl border border-red-100">
          ...
          <p className="text-base font-bold text-red-900">₩{formatCurrency(maxPrice)}</p>
        </div>
        <div className="p-3 bg-blue-50 rounded-xl border border-blue-100">
          <p className="text-[10px] text-blue-600 font-semibold mb-1 uppercase">최저가</p>
          <p className="text-base font-bold text-blue-900">₩{formatCurrency(minPrice)}</p>
        </div>
      </div>
            ...
            {data.length >= 2 ? (
              <div className="flex items-center gap-1.5 mt-0.5">
                ...
                <span className="text-[10px] text-gray-400 ml-1">
                  (이전 입고가 ₩{formatCurrency(data[data.length - 2].price)} 대비)
                </span>
              </div>
            ) : (
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">
              총 <span className="font-bold text-emerald-600">{inboundCount}회</span>의 입고 내역이
              분석되었습니다.
            </p>
          </div>
        </div>
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5 leading-relaxed">
              본 분석은 <span className="font-medium text-gray-700">FIFO (선입선출)</span> 방식을
              사용하여 실제 먼저 들어온 재고의 단가를 우선적으로 적용합니다.
            </p>
          </div>
        </div>
      </div>
