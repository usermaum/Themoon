...
export default function RoastingHistory() {
    ...
    useEffect(() => {
        const fetchLogs = async () => {
            try {
                const data = await RoastingAPI.getHistory({ limit: 5 });
                setLogs(data);
            } catch (error) {
    ...
    if (loading) {
        return (
            <div className="flex justify-center items-center py-20">
                <Loader2 className="w-8 h-8 text-latte-400 animate-spin" />
            </div>
        );
    }
    ...
    if (logs.length === 0) {
        return (
            <MascotStatus
                variant="sleep"
                title="아직 기록된 로스팅 이력이 없습니다"
                description="첫 로스팅을 시작하고 생산 데이터를 기록해보세요. 관리자 냥이가 기다리고 있어요!"
                className=""
            />
        );
    }
    ...
    return (
        <div className="space-y-6">
                        ...
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                            ...
                            <div className="flex items-center gap-4">
                                <div className="flex flex-col">
                                    <span className="text-xs font-bold text-latte-400 font-mono mb-1">
                                        {log.batch_no}
                                    </span>
                            ...
                            <div className="flex items-center gap-6 md:gap-8 bg-latte-50/50 md:bg-transparent p-3 md:p-0 rounded-xl">
                                <div className="flex flex-col items-center md:items-end">
                                    <span className="text-[10px] uppercase font-bold text-latte-400 tracking-tighter mb-1 select-none flex items-center gap-1">
                                        <Scale className="w-3 h-3" /> Production
                                    </span>
                                ...
                                <div className="flex flex-col items-center md:items-end">
                                    <span className="text-[10px] uppercase font-bold text-latte-400 tracking-tighter mb-1 select-none flex items-center gap-1">
                                        <TrendingDown className="w-3 h-3 text-amber-500" /> Loss
                                    </span>
            ...
            <div className="flex justify-center mt-8">
                <button className="text-sm font-bold text-latte-400 hover:text-latte-600 flex items-center gap-2 transition-colors">
