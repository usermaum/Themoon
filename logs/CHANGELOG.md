...
### 🐛 패치 (Bug Fix): 세션 관리 시스템을 어제의 공식 파일로 통일
...
### 🐛 패치 (Bug Fix): UI 버그 수정 및 사이드바 컴포넌트 분리 (v1.6.0)
...
### ✨ 마이너 업데이트 (Minor Update): Phase 3 - 활성 페이지 자동 감지 기능 구현 (v1.5.0)
...
### ✨ 마이너 업데이트 (Minor Update): Claude Desktop 스타일 사이드 메뉴 UI 개선
...
### ✨ 마이너 업데이트 (Minor Update): 다중 언어 지원(i18n) 시스템 구현
