...
### ✨ 인바운드 UI 고도화 및 이미지 최적화 전략 수립

#### 💅 UI/UX Improvements
- **인바운드(입고) 버튼 및 인터랙션 개선**:
    - 하단 저장 버튼 명칭 변경: "입고 확정 및 저장" → "**저장**" (심플하고 명확한 UX).
    - 항목 삭제 버튼 강화: 쓰레기통 아이콘(`Trash2`) 및 레드 컬러 테마를 적용하여 시인성 확보.
- **자동 초기화 (Auto Reset)**:
    - 데이터 저장 성공 시 입력 폼 전체 및 업로드 소스(이미지 미리보기, 파일, URL 등)를 즉시 초기화하여 연속 작업 편의성 증대.
- **아이콘 통일**:
    - 사이드바의 'Inbound' 아이콘을 명세서 페이지와 동일한 `FileText` 아이콘으로 변경하여 시각적 일관성 확보.

#### 📝 Documentation & Planning
- **이미지 최적화 처리 플랜 수립**:
    - 대용량 명세서 이미지의 저장소 부담을 줄이기 위한 압축 및 썸네일 자동 생성 전략 문서화.
    - 향후 Invoice 리스트 및 조회 기능과의 연동을 위한 기술적 로드맵 마련.

## [0.4.1] - 2025-12-22

### ✨ UI 고도화 및 스마트 이미지 유효성 검사 도입

#### 🚀 New Features
- **스마트 이미지 유효성 검사 (AI Validation)**:
  - Gemini AI를 활용하여 업로드된 이미지가 실제 비즈니스 문서(명세서/영수증)인지 사전에 판독하는 로직 도입.
  - 관계없는 이미지(유튜브 스크린샷 등) 업로드 시 서버 저장을 차단하고 사용자에게 즉각적인 경고 메시지 출력.
  - `OCRService` 프롬프트 고도화 및 `analyze` 엔드포인트 내 'Validate-Before-Save' 로직 적용.

#### 💅 UI/UX Improvements
- **인바운드 업로드 UI 전면 개편**:
  - 기존의 투박한 파일 선택 버튼을 세련된 **커스텀 점선 박스(Dashed Box)** 형태의 업로드 영역으로 교체.
  - 파일 선택 시 파일명과 용량이 표시되며, 호버 및 클릭 인터랙션 강화.
  - '붙여넣기' 및 'URL 입력' 탭의 디자인을 새로운 가이드라인(라운드 `1em`, Latte 톤)에 맞춰 통일.
- **아이콘 및 스타일 동기화**:
  - 대시보드의 '블렌드 레시피' 아이콘을 사이드바와 동일한 `Layers` 아이콘으로 일치시켜 일관성 확보.
  - 대시보드 내 모든 테이블 및 카드의 외곽 라운드 값을 `1em`으로 표준화하여 부드러운 통일감 부여.
...
- **하이브리드 동기화 시스템**: 웹/앱 사용량 수동 입력 + CLI 자동 추적
  ...
  - CLI 메시지 증가분 자동 카운트
  - 리셋 시간 자동 계산 및 자동 초기화
- **경고 수준 시각화**: 💎(0-69%) / 🟡(70-89%) / 🔴(90-100%)
- **statusline 출력 개선**: 사용 퍼센트 및 리셋 시간 표시
- **파일 변경**:
  - `.claude/claude_usage_api.py`: baseline 관리 및 자동 추적 로직 추가
  - `.claude/statusline.py`: --sync 명령 추가
  - `.claude/settings.local.json`: refreshInterval 설정 추가
...
- Bean Category Filtering: Beans 페이지 탭 추가 (전체/생두/원두/블렌드)
- Smart Empty State: 검색 조건별 맞춤 액션 버튼
- Multilingual Support: 한글/영문 다국어 표시 및 검색
- Inventory Search: 재고 관리 페이지 실시간 검색
...
- **Inventory Search**: 재고 관리 페이지에 실시간 검색 기능 도입.
  ...
  - 개선: 검색어 입력 시 'X' 초기화 버튼 제공으로 사용자 경험 향상.
- **UI/UX Refinement**:
  - 탭 메뉴 전환 애니메이션 적용 (Inventory, Beans 페이지).
  - 디자인 샘플과 동일한 `slide-in-from-bottom` 효과로 자연스럽게 전환.
