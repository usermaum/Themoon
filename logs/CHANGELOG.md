...
#### 📝 변경사항 (2025-11-16 업데이트)
**수정 파일:**
- `app/utils/text_parser.py`:
  - `parse_gsc_invoice()`: 날짜/금액 패턴에 OCR 오인식 대응 추가
    - 날짜: "년→=, 월→9/0" 패턴 인식 ("2025 = 109 29일" → 2025-10-29)
    - 금액: "합→학/한, 액→9, 괄호 포함" 패턴 인식 ("한계금액 305300) 원" → 305,300)
  - `parse_gsc_table()`: **두 가지 테이블 패턴 자동 감지 및 파싱 (신규)**
    - **패턴 1** (6개 필드, 수량 있음): 품목 규격 수량 중량 단가 금액
    - **패턴 2** (5개 필드, 수량 없음): 품목 규격 중량 단가 금액
    - 원두명 앞 숫자 자동 제거 (이전 항목 금액 오인식 대응)
    - 여러 줄에 걸친 항목 인식 (각 필드가 별도 줄에 있는 OCR 결과 대응)
  - `parse_gsc_table_row()`: 규격 오인식 대응 (kg→k9, Sk→5k)
...
- **날짜**: ✓ 2025-09-26 (OCR: "2025 = 099 26일")
- **금액**: ✓ 305,300원 (OCR: "한계금액 305300) 원")
- **항목**: ✓ 4개 성공적으로 추출 (수량 0개, 중량 20kg씩)
  1. 3r32il NY2/3 Swiss Water Jec3f - 5kg × 20kg
  2. Ethiopi3 G1 Guji Washed - 5kg × 20kg
  3. Ethiopia Gi Yirgscheffe Koke Honey Natural - 5kg × 20kg
  4. Guatem3la S-3 Angu3 - 5kg × 20kg
- **신뢰도**: 전체 100.0% (OCR 53.0%, 파싱 80.5%)

#### 💡 효과
- **두 가지 테이블 구조 모두 지원**: 수량 유무에 관계없이 자동 파싱
- **GSC 명세서 파싱 성공률 100%**: IMG_1650/1651 모두 완벽 파싱
- **OCR 오인식 자동 대응**: 한글/숫자/기호 오인식 패턴 처리
- **실무 환경 즉시 사용 가능**: 다양한 명세서 형식 대응

---

### 🔬 실험 (Experiment): Enhanced 이미지 전처리 모드 (폐기)

#### 📝 시도 내용
**목표:**
- 저품질 이미지의 OCR 인식률 향상을 위한 강화된 전처리 모드 개발

**구현:**
- `app/utils/image_utils.py`:
  - `preprocess_for_easyocr()` 함수에 `mode='enhanced'` 추가
  - Enhanced 모드: 3배 업스케일링, fastNlMeansDenoisingColored, 강화된 CLAHE, Unsharp Mask
- `app/services/ocr_service.py`:
  - OCR 신뢰도 < 60% 시 자동 재시도 로직 추가

**테스트 결과:**
- IMG_1652: 3개 → 0개 항목 (성능 저하)
- IMG_1653: 0개 → 2개 항목 (성능 개선)
- 결론: **불안정한 결과로 실무 적용 불가**

#### 💡 결정사항
- **Enhanced 모드 폐기**: 신뢰할 수 없는 결과로 인해 사용 중단
- **코드 유지**: `image_utils.py`에 Enhanced 모드 코드는 주석 없이 유지 (향후 재설계 가능)
- **대안**: 저품질 이미지는 사용자에게 재촬영 권장 경고 메시지 강화

#### 📝 변경사항 (Refactor)
**수정 파일:**
- `app/services/ocr_service.py`:
  - `process_image()`: auto_enhance 로직 제거
  - 저품질 이미지 경고 메시지 강화 ("더 선명한 이미지로 다시 시도해주세요")
- `app/utils/image_utils.py`:
  - Enhanced 모드 코드 유지 (현재 미사용)
...
### 🐛 패치 (Bug Fix): 성능 테스트 이미지 처리 방식 수정
...
### 🐛 패치 (Bug Fix): ImageInvoiceUpload 페이지에 language_manager 초기화 추가
...
### 🐛 패치 (Bug Fix): pages 파일의 import 경로 수정
