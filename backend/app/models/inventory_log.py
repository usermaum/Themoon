...
class InventoryLog(Base):
    ...
    inbound_document = relationship(
        "app.models.inbound_document.InboundDocument", back_populates="inventory_logs"
    )

    roasting_log_id = Column(Integer, ForeignKey("roasting_logs.id"), nullable=True, comment="로스팅 배치 ID")
    roasting_log = relationship(
        "app.models.roasting_log.RoastingLog", back_populates="inventory_logs"
    )
