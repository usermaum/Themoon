...
class Bean(Base):
    ...
    parent_bean_id = Column(
        Integer, ForeignKey("beans.id"), nullable=True, comment="원재료 생두 ID"
    )
    ...
    expected_loss_rate = Column(
        Float, default=0.15, nullable=False, comment="예상 로스팅 손실률 (0.0 ~ 1.0)"
    )
    ...
    parent_bean = relationship(
        "Bean", remote_side=[id], backref=backref("roasted_variants", lazy="dynamic")
    )

    # 재고 로그
    inventory_logs = relationship(
        "InventoryLog", back_populates="bean", cascade="all, delete-orphan"
    )
