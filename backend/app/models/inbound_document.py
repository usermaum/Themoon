...
class InboundDocument(Base):
    ...
    contract_number = Column(
        String, unique=True, index=True, nullable=True, comment="Contract/Order Number"
    )
    ...
    inventory_logs = relationship(
        "app.models.inventory_log.InventoryLog", back_populates="inbound_document"
    )

    # New relationships for OCR data (Option B redesign)
    detail = relationship(
        "app.models.inbound_document_detail.InboundDocumentDetail",
        back_populates="inbound_document",
        uselist=False,
    )
    receiver = relationship(
        "app.models.inbound_receiver.InboundReceiver",
        back_populates="inbound_document",
        uselist=False,
    )
    items = relationship(
        "app.models.inbound_item.InboundItem",
        back_populates="inbound_document",
        order_by="app.models.inbound_item.InboundItem.item_order",
    )
