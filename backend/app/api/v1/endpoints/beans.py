...
def read_beans(
    ...
    roast_level: Optional[str] = Query(None, description="로스팅 레벨 필터 (Green, Roasted, or specific level)"),
    type: Optional[str] = Query(None, description="원두 유형 (GREEN_BEAN, ROASTED_BEAN)"),
    db: Session = Depends(get_db)
):
    """원두 목록 조회"""
    skip = (page - 1) * size
    beans = bean_service.get_beans(db, skip=skip, limit=size, search=search, roast_level=roast_level, bean_type=type)
    total = bean_service.get_beans_count(db, search=search, roast_level=roast_level, bean_type=type)
